...
const Navbar = () => {
  ...
  return (
    <div className='w-full h-20 lg:px-32 px-6 flex justify-between items-center text-white absolute top-0 z-50'>
     ...
     <Image src='/logo.webp' alt='logo' width={70} height={10} className='brightness-105 rounded-full bg-white scale-75 border border-neutral-400'/>
      {isMobile ? (
        <div className='relative'>
          ...
          />
          {show && (
            <div className='absolute right-0 mt-4 w-40 bg-[#C28D35] text-white rounded-lg shadow-lg py-2'>
              {Nav.map((item, index) => (
                <Link
